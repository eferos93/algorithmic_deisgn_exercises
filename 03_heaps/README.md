...
The solution to the theoretical exercises can be found in the pdf file.
Regarding the practical exercise, I did not need to use two arrays, but just one (`key_pos`) was
sufficient. For compiling just run cmake as described below and then run make.
For the plots run `python plot.py` inside the benchmark_plot folder (requires pandas library).
